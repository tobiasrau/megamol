...
AbstractOSPRayTestRenderer::AbstractOSPRayTestRenderer()
    ...
    , resizeVectorsSlot(
          "resizeVectors", "Flag indicates if vectors used in contour computations should be resized to save memory")
    , computeSingularitiesSlot(
          "computeSingularities", "Flag indicating if spherical triangle singularities are considered")
    ...
    , rendererTypeSlot("RendererType", "Switch the renderer type") {
    ...
    enum rt { SES = 0, SCIVIS = 1 };
    megamol::core::param::EnumParam* rtypeParam = new megamol::core::param::EnumParam(rt::SES);
    rtypeParam->SetTypePair(rt::SES, "SES");
    rtypeParam->SetTypePair(rt::SCIVIS, "SciVis");
    this->rendererTypeSlot << rtypeParam;
    this->MakeSlotAvailable(&this->rendererTypeSlot);
...
AbstractOSPRayTestRenderer::~AbstractOSPRayTestRenderer() { this->Release(); }
...
bool AbstractOSPRayTestRenderer::InitOSP() {
    ...
    ospDeviceSetErrorFunc(this->device,
        [](OSPError e, const char* msg) { std::cout << "OSPRAY ERROR [" << e << "]: " << msg << std::endl; });

    bool success = false;
    if (OSPRaySESModuleLoader::LoadModule()) {
        if (this->rendererTypeSlot.Param<core::param::EnumParam>()->Value() == 0) {
    ...
    try {
        if (!this->osprayShader.Create(vert.Code(), vert.Count(), frag.Code(), frag.Count())) {
            vislib::sys::Log::DefaultLog.WriteMsg(
                vislib::sys::Log::LEVEL_ERROR, "Unable to compile ospray ses shader: Unknown error\n");
            return false;
        }
        ...
        vislib::sys::Log::DefaultLog.WriteMsg(
            vislib::sys::Log::LEVEL_ERROR, "Unable to compile ospray ses shader: %s\n", e.GetMsgA());
        return false;
    } catch (...) {
        vislib::sys::Log::DefaultLog.WriteMsg(
            vislib::sys::Log::LEVEL_ERROR, "Unable to compile ospray ses shader: Unknown exception\n");
        return false;
    }
...
void AbstractOSPRayTestRenderer::UpdateCameraAndRenderer(
    const vislib::SmartPtr<vislib::graphics::CameraParameters>& camParams, bool clearBuffer) {
...
OSPModel AbstractOSPRayTestRenderer::GetScene() { return this->scene; }

OSPRenderer AbstractOSPRayTestRenderer::GetRenderer() { return this->renderer; }

OSPGeometry AbstractOSPRayTestRenderer::CreateSurface() { return ospNewGeometry("ses_surface"); }

void AbstractOSPRayTestRenderer::AddGeometry(OSPGeometry geometry) { this->geometries.push_back(geometry); }

void AbstractOSPRayTestRenderer::FinalizeCustomGeometry(std::map<std::string, std::vector<OSPGeometry>>& geoVec) {
    // for (OSPGeometry g : geoVec) {
    //    ospAddGeometry(this->scene, g);
    //}
    for (auto geo : geoVec) {
        for (size_t i = 0; i < geo.second.size(); i++) {
            ospAddGeometry(this->scene, geo.second[i]);
        }
    }
...
int AbstractOSPRayTestRenderer::GetGeometryCnt() { return static_cast<int>(this->geometries.size()); }
...
void AbstractOSPRayTestRenderer::ReadInterfaceRenderer() {
    ...
    if (this->rendererTypeSlot.Param<core::param::EnumParam>()->Value() == 0) {
        ...
        this->rendererParamValues.useHeadLight =
            this->useHeadlightSlot.Param<megamol::core::param::BoolParam>()->Value();
        const vislib::math::Vector<float, 3> lightDir(
            this->lightDirSlot.Param<megamol::core::param::Vector3fParam>()->Value());

        this->rendererParamValues.lightDir = normalize(vec3f{lightDir.X(), lightDir.Y(), lightDir.Z()});
        this->rendererParamValues.lightDiffuseWeight =
            this->lightDiffuseWeightSlot.Param<megamol::core::param::FloatParam>()->Value();
        this->rendererParamValues.lightAmbientWeight =
            this->lightAmbientWeightSlot.Param<megamol::core::param::FloatParam>()->Value();
        this->rendererParamValues.lightExponent =
            this->lightExponentSlot.Param<megamol::core::param::FloatParam>()->Value();
        this->rendererParamValues.alpha = this->alphaSlot.Param<megamol::core::param::FloatParam>()->Value();

        this->rendererParamValues.useCuttingPlane =
            this->useCuttingPlaneSlot.Param<megamol::core::param::BoolParam>()->Value();
        const vislib::math::Vector<float, 4> plane(
            this->cuttingPlaneSlot.Param<megamol::core::param::Vector4fParam>()->Value());
        ...
        this->rendererParamValues.smallCavities =
            this->smallCavitiesSlot.Param<megamol::core::param::BoolParam>()->Value();
        ...
        const vislib::math::Vector<float, 3> blendingColor =
            this->blendingColorSlot.Param<megamol::core::param::Vector3fParam>()->Value();

        this->rendererParamValues.blendingColor = vec3f{blendingColor.X(), blendingColor.Y(), blendingColor.Z()};
        this->rendererParamValues.blendingWeight =
            this->blendingWeightSlot.Param<megamol::core::param::FloatParam>()->Value();
    } else {
        std::vector<OSPLight> lights(1);
        lights[0] = ospNewLight3("ambient");
        ospSetObject(this->renderer, "lights", *lights.data());
    }
}

void AbstractOSPRayTestRenderer::SetRenderParams(
    const vislib::SmartPtr<vislib::graphics::CameraParameters>& camParams) {
...
void AbstractOSPRayTestRenderer::SetSurfaceParams(OSPGeometry sesGeometry) {
    ...
    ospSet1i(
        sesGeometry, "skip_contour_singularities", static_cast<int>(this->surfaceParamValues.skipContourSingularities));
...
void AbstractOSPRayTestRenderer::ReadInterfaceSurface() {
    ...
    this->surfaceParamValues.skipContourSingularities =
        this->skipContourSingularitiesSlot.Param<megamol::core::param::BoolParam>()->Value();
    this->surfaceParamValues.computeSingularities =
        this->computeSingularitiesSlot.Param<megamol::core::param::BoolParam>()->Value();
    ...
    this->surfaceParamValues.useIterativeMethod =
        this->useIterativeMethodSlot.Param<megamol::core::param::BoolParam>()->Value();

    this->surfaceParamValues.epsilonCuttingThreshold =
        this->epsilonCuttingThresholdSlot.Param<megamol::core::param::FloatParam>()->Value();
    this->surfaceParamValues.epsilonSelfIntersect =
        this->epsilonSelfIntersectSlot.Param<megamol::core::param::FloatParam>()->Value();

    this->surfaceParamValues.renderContourArcs =
        this->renderContourArcsSlot.Param<megamol::core::param::BoolParam>()->Value();
    this->surfaceParamValues.mode =
        static_cast<ComputationMode>(this->modeSlot.Param<megamol::core::param::EnumParam>()->Value());
}
...
bool AbstractOSPRayTestRenderer::RenderPre() {
    ...
    if (this->modeSlot.IsDirty() || this->probeRadiusSlot.IsDirty() || this->skipContourSingularitiesSlot.IsDirty() ||
        ...
        this->debugFlagSlot.IsDirty() || this->useIterativeMethodSlot.IsDirty() ||
        this->epsilonCuttingThresholdSlot.IsDirty() || this->epsilonSelfIntersectSlot.IsDirty() ||
        this->renderContourArcsSlot.IsDirty()) {
    ...
    this->rendererParamValues.dirty = false;
    if (this->epsilonSlot.IsDirty() || this->alphaSlot.IsDirty() || this->rhoSlot.IsDirty() ||
        this->tauSlot.IsDirty() || this->useCuttingPlaneSlot.IsDirty() || this->cuttingPlaneSlot.IsDirty() ||
        this->lightDirSlot.IsDirty() || this->useHeadlightSlot.IsDirty() || this->lightDiffuseWeightSlot.IsDirty() ||
        this->lightAmbientWeightSlot.IsDirty() || this->lightExponentSlot.IsDirty() || this->useAOOMSlot.IsDirty() ||
        this->smallCavitiesSlot.IsDirty() || this->aoDistanceSlot.IsDirty() || this->aoSamplesSlot.IsDirty() ||
